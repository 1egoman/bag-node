...
(function() {
  ...
  angular.module('bag.controllers', ['btford.socket-io', 'bag.authorization', 'bag.controllers.onboarding', 'bag.controllers.account', 'bag.controllers.stores_picker', 'bag.controllers.tab_bag', 'bag.controllers.tab_recipe', 'bag.controllers.tab_picks', 'bag.controllers.item_info', 'bag.controllers.new_foodstuff', 'bag.controllers.new_recipe', 'bag.controllers.recipe_card', 'bag.controllers.login']).controller('RecipeListCtrl', function($scope, socket, $ionicSlideBoxDelegate) {
...
(function() {
  angular.module('bag.controllers.login', []).controller('authCtrl', function($scope, $http, $state, socket) {
    socket.on("login:callback", function(data) {
      if (data.err) {
        return alert("Those login credentials don't match what we have on file. Give it another try?");
      } else {
